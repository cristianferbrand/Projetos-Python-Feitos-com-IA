...
def process_directory(directory_path, aliquota_pis, aliquota_cofins):
    registros_output.delete(1.0, tk.END)  # Limpa a tela antes de exibir novos registros
    for filename in os.listdir(directory_path):
        if filename.endswith(".xml"):
            file_path = os.path.join(directory_path, filename)
            registros = parse_xml(file_path, aliquota_pis, aliquota_cofins)
            registros_output.insert(tk.END, f"Processado: {filename}\n")
            registros_output.insert(tk.END, "\n".join(registros) + "\n\n")
...
entry_aliquota_pis.insert(0, "0,00")  # Valor padrão
...
entry_aliquota_cofins.insert(0, "0,00")  # Valor padrão
entry_aliquota_cofins.pack()

# Área de saída para exibição dos registros
registros_output = scrolledtext.ScrolledText(root, width=70, height=10, font=("Arial", 10))
registros_output.pack(pady=10)
