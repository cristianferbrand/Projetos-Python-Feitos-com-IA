...
def calcular_difal():
    ...
    try:
        ...
        try:
            aliq_interna = obter_valor_para_calculo(entry_aliq_interna.get()) / 100
        except ValueError:
            campos_invalidos.append("% Alíquota Interna")

        try:
            aliq_interestadual = obter_valor_para_calculo(combo_aliq_interestadual.get()) / 100
        except ValueError:
            campos_invalidos.append("Alíquota Interestadual")

        try:
            fcp = obter_valor_para_calculo(combo_fcp.get()) / 100
        except ValueError:
        ...
        label_resultado_total.config(text=f"R$ {difal + valor_fcp:,.2f}".replace('.', 'X').replace(',', '.').replace('X', ','))
...
def limpar_campos():
    ...
    label_resultado_base_icms_destino.config(text="R$ 0,00")
    label_resultado_valor_icms_destino.config(text="R$ 0,00")
    label_resultado_total.config(text="R$ 0,00")
...
root = tk.Tk()
root.title("Calculadora DIFAL Por Fora")
...
# Entradas
ttk.Label(input_frame, text="% Alíquota Interna:").grid(row=0, column=0, sticky=tk.W, pady=5)
entry_aliq_interna = ttk.Entry(input_frame, width=20)
entry_aliq_interna.grid(row=0, column=1, sticky=tk.E, pady=5)
entry_aliq_interna.bind('<KeyRelease>', formatar_valor_dinamico)
entry_aliq_interna.bind('<Return>', on_enter)

ttk.Label(input_frame, text="Alíquota Interestadual:").grid(row=1, column=0, sticky=tk.W, pady=5)
combo_aliq_interestadual = ttk.Combobox(input_frame, values=["4", "7", "12"], width=17)
combo_aliq_interestadual.grid(row=1, column=1, sticky=tk.E, pady=5)
combo_aliq_interestadual.current(0)
combo_aliq_interestadual.bind('<Return>', on_enter)

ttk.Label(input_frame, text="% Fundo de Combate à Pobreza:").grid(row=2, column=0, sticky=tk.W, pady=5)
combo_fcp = ttk.Combobox(input_frame, values=["0", "1", "1.6", "2"], width=17)
combo_fcp.grid(row=2, column=1, sticky=tk.E, pady=5)
combo_fcp.current(0)
combo_fcp.bind('<Return>', on_enter)

ttk.Label(input_frame, text="Valor Produto:").grid(row=3, column=0, sticky=tk.W, pady=5)
entry_valor_produto = ttk.Entry(input_frame, width=20)
entry_valor_produto.grid(row=3, column=1, sticky=tk.E, pady=5)
entry_valor_produto.bind('<KeyRelease>', formatar_valor_dinamico)
entry_valor_produto.bind('<Return>', on_enter)

ttk.Label(input_frame, text="Valor do IPI:").grid(row=4, column=0, sticky=tk.W, pady=5)
entry_valor_ipi = ttk.Entry(input_frame, width=20)
entry_valor_ipi.grid(row=4, column=1, sticky=tk.E, pady=5)
entry_valor_ipi.bind('<KeyRelease>', formatar_valor_dinamico)
entry_valor_ipi.bind('<Return>', on_enter)

ttk.Label(input_frame, text="Valor do Frete:").grid(row=5, column=0, sticky=tk.W, pady=5)
entry_valor_frete = ttk.Entry(input_frame, width=20)
entry_valor_frete.grid(row=5, column=1, sticky=tk.E, pady=5)
entry_valor_frete.bind('<KeyRelease>', formatar_valor_dinamico)
entry_valor_frete.bind('<Return>', on_enter)

ttk.Label(input_frame, text="Outras Despesas Acessórias:").grid(row=6, column=0, sticky=tk.W, pady=5)
entry_outras_despesas = ttk.Entry(input_frame, width=20)
entry_outras_despesas.grid(row=6, column=1, sticky=tk.E, pady=5)
entry_outras_despesas.bind('<KeyRelease>', formatar_valor_dinamico)
entry_outras_despesas.bind('<Return>', on_enter)

ttk.Label(input_frame, text="Descontos:").grid(row=7, column=0, sticky=tk.W, pady=5)
entry_descontos = ttk.Entry(input_frame, width=20)
entry_descontos.grid(row=7, column=1, sticky=tk.E, pady=5)
entry_descontos.bind('<KeyRelease>', formatar_valor_dinamico)
entry_descontos.bind('<Return>', on_enter)

# Botão Calcular
btn_calcular = ttk.Button(input_frame, text="Calcular", command=calcular_difal)
btn_calcular.grid(row=8, column=0, pady=10)

# Botão Limpar
btn_limpar = ttk.Button(input_frame, text="Limpar", command=limpar_campos)
btn_limpar.grid(row=8, column=1, pady=10)
...
ttk.Label(result_frame, text="Valor DIFAL:").grid(row=1, column=0, sticky=tk.W, pady=5)
label_resultado_difal = ttk.Label(result_frame, text="R$ 0,00", font=("Arial", 10, "bold"))
label_resultado_difal.grid(row=1, column=1, sticky=tk.E, pady=5)

ttk.Label(result_frame, text="Valor Fundo de Combate à Pobreza:").grid(row=2, column=0, sticky=tk.W, pady=5)
label_resultado_fcp = ttk.Label(result_frame, text="R$ 0,00", font=("Arial", 10, "bold"))
label_resultado_fcp.grid(row=2, column=1, sticky=tk.E, pady=5)

ttk.Label(result_frame, text="Base ICMS UF Destino:").grid(row=3, column=0, sticky=tk.W, pady=5)
label_resultado_base_icms_destino = ttk.Label(result_frame, text="R$ 0,00", font=("Arial", 10, "bold"))
label_resultado_base_icms_destino.grid(row=3, column=1, sticky=tk.E, pady=5)

ttk.Label(result_frame, text="Valor ICMS UF Destino:").grid(row=4, column=0, sticky=tk.W, pady=5)
label_resultado_valor_icms_destino = ttk.Label(result_frame, text="R$ 0,00", font=("Arial", 10, "bold"))
label_resultado_valor_icms_destino.grid(row=4, column=1, sticky=tk.E, pady=5)

ttk.Label(result_frame, text="Valor Total:").grid(row=5, column=0, sticky=tk.W, pady=5)
label_resultado_total = ttk.Label(result_frame, text="R$ 0,00", font=("Arial", 10, "bold"))
label_resultado_total.grid(row=5, column=1, sticky=tk.E, pady=5)
