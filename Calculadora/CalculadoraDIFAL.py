...
def carregar_tabela():
    caminho_arquivo = os.path.join(os.path.dirname(__file__), 'bd\\Aliquota_Interestadual.xlsx')
    if not os.path.exists(caminho_arquivo):
        raise FileNotFoundError(f"Arquivo '{caminho_arquivo}' não encontrado. Certifique-se de que está na pasta bd.")
    df = pd.read_excel(caminho_arquivo, index_col=0)
    return df

def buscar_aliquotas(df, origem, destino):
    try:
        aliquota_interna = df.loc[destino, destino]
...
def mostrar_aliquotas():
    ...
    if origem and destino:
        ...
        if interna is not None and interestadual is not None:
            resultado_var.set(f"Alíquota Interna ({destino}): {interna}%\nAlíquota Interestadual ({origem} -> {destino}): {interestadual}%")
        else:
...
try:
    ...
    messagebox.showerror("Erro ao Carregar Tabela", str(e))
    exit()

# Configuração da janela principal
root = tk.Tk()
root.title("Calculadora DIFAL e Consulta de Alíquotas")
...
# Frame para consulta de alíquotas
consulta_frame = ttk.LabelFrame(frame, text="Consulta Aliquota", padding="10")
consulta_frame.grid(row=1, column=0, padx=10, pady=10, sticky=tk.N)

ttk.Label(consulta_frame, text="UF Origem:").grid(row=0, column=0, padx=10, pady=5)
combo_origem = ttk.Combobox(consulta_frame, values=list(tabela_aliquotas.index))
combo_origem.grid(row=0, column=1, padx=10, pady=5)

ttk.Label(consulta_frame, text="UF Destino:").grid(row=1, column=0, padx=10, pady=5)
combo_destino = ttk.Combobox(consulta_frame, values=list(tabela_aliquotas.index))
combo_destino.grid(row=1, column=1, padx=10, pady=5)

ttk.Button(consulta_frame, text="Consultar", command=mostrar_aliquotas).grid(row=2, column=0, columnspan=2, pady=10)

resultado_var = StringVar()
ttk.Label(consulta_frame, textvariable=resultado_var, justify="left").grid(row=3, column=0, columnspan=2, padx=10, pady=10)
