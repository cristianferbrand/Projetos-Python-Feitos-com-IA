...
- Reutiliza o mesmo settings.json (./config/settings.json), adicionando chaves SMTP_*.
...
# ====== UI (Flet) ======
APP_TITLE = "Email Sender (DB) — Acronis HOS"
...
def main(page: ft.Page):
    ...
    page_size_dd = ft.Dropdown(
        label="Por página",
        width=140,
        options=[ft.dropdown.Option(str(n)) for n in (25, 50, 100, 200, 400, 600)],
        value="100"
    )
    ...
    def fill_table_from_db():
        ...
        try:
            ...
            for i, r in enumerate(rows):
                chk = ft.Checkbox(value=False, on_change=lambda e, ii=i: toggle_select(ii))
    ...
    page.add(
        ft.Column([
            ...
            toolbar,
            ft.Container(table, expand=True, bgcolor=ft.Colors.WHITE, padding=10, border_radius=8),
            pager,
