...
def main(page: ft.Page):
    page.title = "E-mail Sender — Cloud Backup HOS (DB) v1.0 | Desenvolvido por Cristianfer Brand"
    ...
    selected_label = ft.Text("Selecionados: 0")
    ...
    def open_settings_view(_e=None):
        ...
        tf_db   = ft.TextField(label="DATABASE_URL (somente sqlite:///)", value=str(settings.get("DATABASE_URL","")), hint_text="vazio = usar ./data/acronis_database.db", width=520)
    ...
    def fill_table_from_db():
        ...
        try:
            ...
            state["rows"] = rows
            # Mantém seleção válida e atualiza contador
            state["selected"] = {i for i in state["selected"] if 0 <= i < len(rows)}
            selected_label.value = f"Selecionados: {len(state['selected'])}"
    ...
    def on_key(e: ft.KeyboardEvent):
        try:
            k = (e.key or '').lower()
            if getattr(e, 'ctrl', False) and k in ('a', 'keya'):
                select_all(None)
        except Exception:
            pass
    ...
    toolbar = ft.Row([
        ...
        ft.Container(content=selected_label, padding=ft.padding.only(left=8, top=8)),
    ], spacing=8, wrap=True)
