# -*- coding: utf-8 -*-
"""
App de Solicitações para Setor de LEADTECH (Flet) — (Abas + Detalhe + Chat + Anexos)
...
DB_PATH = os.path.join(DATA_DIR, "leadtech_requests.db")  # reaproveita o mesmo DB
...
# ---------------------------------------------------------------------------
# Esquema: usuários, grupos, associação, solicitações LEADTECH, mensagens e arquivos
...
CREATE TABLE IF NOT EXISTS leadtech_requests (
    id BIGSERIAL PRIMARY KEY,
    title TEXT NOT NULL,
    description TEXT,
    cliente TEXT,
...
CREATE TABLE IF NOT EXISTS leadtech_request_messages (
    id BIGSERIAL PRIMARY KEY,
    request_id BIGINT NOT NULL REFERENCES leadtech_requests(id) ON DELETE CASCADE,
    sender_id BIGINT NOT NULL REFERENCES users(id) ON DELETE SET NULL,
    message TEXT NOT NULL,
    created_at TEXT NOT NULL
);

CREATE TABLE IF NOT EXISTS leadtech_request_files (
    id BIGSERIAL PRIMARY KEY,
    request_id BIGINT NOT NULL REFERENCES leadtech_requests(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS leadtech_request_reads (
    request_id BIGINT NOT NULL REFERENCES leadtech_requests(id) ON DELETE CASCADE,
    user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
def ensure_settings_defaults() -> dict:
    s = load_settings()
    default_dsn = "postgresql://leadtech_app:suporte@123@127.0.0.1:5432/leadtech_requests"
    changed = False
...
def _read_database_url_from_settings() -> str:
    s = load_settings()
    return s.get("DATABASE_URL") or os.getenv("DATABASE_URL") or "postgresql://leadtech_app:suporte@123@127.0.0.1:5432/leadtech_requests"
...
def get_pool():
    ...
    url = resolve_database_url(s, default_url="postgresql://leadtech_app:suporte@123@127.0.0.1:5432/leadtech_requests")
...
class DB:
    ...
    def _migrate_indices(self):
        stmts = [
            ...
            "CREATE INDEX IF NOT EXISTS idx_requests_status ON leadtech_requests(status);",
            "CREATE INDEX IF NOT EXISTS idx_requests_created_at ON leadtech_requests(created_at);",
            "CREATE INDEX IF NOT EXISTS idx_requests_taken_by ON leadtech_requests(taken_by);",
            "CREATE INDEX IF NOT EXISTS idx_msg_req ON leadtech_request_messages(request_id);",
            "CREATE INDEX IF NOT EXISTS idx_msg_created_at ON leadtech_request_messages(created_at);",
            "CREATE INDEX IF NOT EXISTS idx_file_req ON leadtech_request_files(request_id);",
            "CREATE INDEX IF NOT EXISTS idx_file_created_at ON leadtech_request_files(created_at);",
            "CREATE INDEX IF NOT EXISTS idx_msg_req_id ON leadtech_request_messages(request_id, id);"
        
            "CREATE INDEX IF NOT EXISTS idx_reads_user ON leadtech_request_reads(user_id);",
            "CREATE INDEX IF NOT EXISTS idx_reads_req ON leadtech_request_reads(request_id);",
            "CREATE INDEX IF NOT EXISTS idx_msg_req_sender_id ON leadtech_request_messages(request_id, sender_id, id);",]
        with self.connect() as con:
            with con.cursor() as cur:
                for s in stmts:
                    cur.execute(s)
    ...
    def _bootstrap_default_groups(self):
        with self.connect() as con:
            with con.cursor() as cur:
                ...
                cur.execute("INSERT INTO groups(name) VALUES (%s) ON CONFLICT (name) DO NOTHING", ("LEADTECH",))
            con.commit()
    ...
    def list_requests(self, *, status: Optional[str] = None, only_mine_opened_by: Optional[int] = None,
        ...
        q = "SELECT * FROM leadtech_requests WHERE 1=1"
        params = []
        if status:
            q += " AND status=%s"
    ...
    def get_request(self, rid: int) -> Optional[Request]:
        with self.connect() as con:
            with con.cursor(row_factory=dict_row) as cur:
                cur.execute("SELECT * FROM leadtech_requests WHERE id=%s", (rid,))
                row = cur.fetchone()
    ...
    def take_request(self, rid: int, user_id: int):
        ...
        with self.connect() as con:
            with con.cursor(row_factory=dict_row) as cur:
                cur.execute("SELECT history FROM leadtech_requests WHERE id=%s", (rid,))
                row = cur.fetchone()
                hist = json.loads(row["history"] or "[]") if row else []
                hist.append({"ts": now, "evt": "em_atendimento", "by": user_id})
            with con.cursor() as cur2:
                cur2.execute(
                    "UPDATE leadtech_requests SET status='em_atendimento', taken_by=%s, taken_at=%s, last_update=%s, history=%s WHERE id=%s",
    ...
    def release_request(self, rid: int, user_id: int):
        ...
        with self.connect() as con:
            ...
            with con.cursor() as cur2:
                cur2.execute(
                    "UPDATE leadtech_requests SET status='aberta', taken_by=NULL, taken_at=NULL, last_update=%s, history=%s WHERE id=%s",
    ...
    def resolve_request(self, rid: int, user_id: int):
        ...
        with self.connect() as con:
            ...
            with con.cursor() as cur2:
                cur2.execute(
                    "UPDATE leadtech_requests SET status='resolvida', closed_at=%s, last_update=%s, history=%s WHERE id=%s",
    ...
    def cancel_request(self, rid: int, user_id: int):
        ...
        with self.connect() as con:
            with con.cursor() as cur1:
                cur1.execute("SELECT history FROM leadtech_requests WHERE id=%s", (rid,))
                row = cur1.fetchone()
                hist = json.loads(row[0] or "[]") if row else []
                hist.append({"ts": now, "evt": "cancelada", "by": user_id})
            with con.cursor() as cur2:
                cur2.execute(
                    "UPDATE leadtech_requests SET status='cancelada', closed_at=%s, last_update=%s, history=%s WHERE id=%s",
                    (now, now, json.dumps(hist, ensure_ascii=False), rid),
                )
                con.commit()
    ...
    def add_message(self, request_id: int, sender_id: int, message: str) -> int:
        ...
        with self.connect() as con:
            with con.cursor() as cur:
                cur.execute(
                    "INSERT INTO leadtech_request_messages(request_id, sender_id, message, created_at) VALUES (%s,%s,%s,%s) RETURNING id",
    ...
    def list_messages(self, request_id: int, limit: int = 200) -> List[Message]:
        with self.connect() as con:
            with con.cursor(row_factory=dict_row) as cur:
                cur.execute(
                    "SELECT * FROM leadtech_request_messages WHERE request_id=%s ORDER BY created_at ASC LIMIT %s",
                    (request_id, limit),
                )
    ...
    def last_message_id(self, request_id: int) -> int:
        with self.connect() as con:
            with con.cursor() as cur:
                cur.execute("SELECT MAX(id) FROM leadtech_request_messages WHERE request_id=%s", (request_id,))
                row = cur.fetchone()
    ...
    def list_messages_since(self, request_id: int, last_id: int, limit: int = 500) -> List[Message]:
        with self.connect() as con:
            with con.cursor(row_factory=dict_row) as cur:
                cur.execute(
                    "SELECT * FROM leadtech_request_messages WHERE request_id=%s AND id>%s ORDER BY id ASC LIMIT %s",
    ...
    def get_last_seen(self, request_id: int, user_id: int) -> int:
        with self.connect() as con:
            with con.cursor() as cur:
                cur.execute(
                    "SELECT last_seen_msg_id FROM leadtech_request_reads WHERE request_id=%s AND user_id=%s",
                    (request_id, user_id),
                )
                row = cur.fetchone()
    ...
    def set_last_seen(self, request_id: int, user_id: int, last_seen_msg_id: int) -> None:
        ...
        with self.connect() as con:
            with con.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO leadtech_request_reads (request_id, user_id, last_seen_msg_id, updated_at)
                    VALUES (%s, %s, %s, %s)
    ...
    def get_unread_count(self, request_id: int, me_id: int, other_id: int) -> int:
        ...
        with self.connect() as con:
            with con.cursor() as cur:
                cur.execute(
                    """
                    SELECT COUNT(*) FROM leadtech_request_messages
                     WHERE request_id=%s
                       AND sender_id=%s
                       AND id > %s
                    """,
    ...
    def get_message_dict(self, msg_id: int) -> Dict[str, Any]:
        with self.connect() as con:
            with con.cursor(row_factory=dict_row) as cur:
                cur.execute("SELECT id, request_id, sender_id, message, created_at FROM leadtech_request_messages WHERE id=%s", (msg_id,))
                r = cur.fetchone()
    ...
    def add_file(self, request_id: int, filename: str, mime: str, data: bytes, uploaded_by: int) -> int:
        ...
        with self.connect() as con:
            with con.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO leadtech_request_files(request_id, filename, mime, size_bytes, blob, uploaded_by, created_at)
    ...
    def list_files(self, request_id: int) -> List[Attachment]:
        with self.connect() as con:
            with con.cursor(row_factory=dict_row) as cur:
                cur.execute("SELECT * FROM leadtech_request_files WHERE request_id=%s ORDER BY created_at ASC", (request_id,))
                def _ts(x): return datetime.fromisoformat(x) if x else None
                return [
                    Attachment(
    ...
    def get_file(self, file_id: int) -> Optional[Attachment]:
        with self.connect() as con:
            with con.cursor(row_factory=dict_row) as cur:
                cur.execute("SELECT * FROM leadtech_request_files WHERE id=%s", (file_id,))
                r = cur.fetchone()
                if not r:
                    return None
                def _ts(x): return datetime.fromisoformat(x) if x else None
    ...
    def delete_file(self, file_id: int) -> None:
        with self.connect() as con:
            with con.cursor() as cur:
                cur.execute("DELETE FROM leadtech_request_files WHERE id=%s", (file_id,))
            con.commit()
...
class LoginView:
    ...
    def _build(self):
        self.page.title = "Login - Solicitações LEADTECH"
        ...
        card = ft.Card(
            ...
            content=ft.Container(
                ...
                content=ft.Column([
                    ft.Text("Solicitações LEADTECH", size=22, weight=ft.FontWeight.W_700, text_align=ft.TextAlign.CENTER),
...
class LEADTECHApp:
    ...
    def _load_minhas_rows(self) -> None:
            ...
            if getattr(self, "current_tab", None) != 3:
                return
            ...
            try:
                if hasattr(self, 'page') and hasattr(self.page, 'update'):
                    ...
                    if start_gen != getattr(self, "_tab_gen", 0) or getattr(self, "current_tab", None) != 3:
                        return
                    self.page.update()
            except Exception:
                pass
    ...
    def __init__(self, page: ft.Page, db: DB, user: User, on_logout: Callable[[], None]):
            ...
            self.page.title = f"Solicitações LEADTECH — {user.username}"
            settings = load_settings()
            theme_mode = settings.get("theme_mode", "dark")
            self.page.theme_mode = ft.ThemeMode.DARK if theme_mode == "dark" else ft.ThemeMode.LIGHT
            self.palette = PALETTE_DARK if theme_mode == "dark" else PALETTE_LIGHT
            ...
            self._fp_chat = None
            # Estado
            self.current_tab = 0  # 0=Home,1=Abrir,2=Fila,3=Minhas,4=Admin
    ...
    async def _fila_timer_loop(self):
            import asyncio
            self._fila_next_refresh_s = 30
            while self._fila_timer_on and getattr(self, "current_tab", None) == 2:
                try:
                    try:
    ...
    async def _minhas_timer_loop(self):
            import asyncio
            self._minhas_next_refresh_s = 30
            while self._minhas_timer_on and getattr(self, "current_tab", None) == 3:
                try:
                    try:
    ...
    def _user_is(self, group_name: str) -> bool:
        ...
        # --- Helpers de permissão e feeleadtechck ---
    ...
    def _can_queue(self) -> bool:
            ...
            return self.user.is_admin or self._user_is("LEADTECH")
    ...
    def _load_fila_rows(self):
            ...
            self._update_fila_badge()
            # Guard final contra renderização atrasada
            if start_gen != getattr(self, "_tab_gen", 0) or getattr(self, "current_tab", None) != 2:
                return
            # Guard final contra renderização atrasada
            if start_gen != getattr(self, "_tab_gen", 0) or getattr(self, "current_tab", None) != 2:
                return
    ...
    def _home_tab(self) -> ft.Control:
            ...
            return ft.Column([
                ft.Row([
                    ft.Text("Bem-vindo(a) ao Solicitações LEADTECH", size=20, weight=ft.FontWeight.W_700),
                ]),
                ...
                ft.ResponsiveRow([
                    ft.Container(
                        content=ft.Column([
                            ...
                            ft.Text("Usuários do grupo SUPORTE podem abrir solicitações para o LEADTECH.", size=12, color=C.GREY_600),
                        ], spacing=4),
                    ...
                    ft.Container(
                        content=ft.Column([
                            ...
                            ft.Text("Usuários do grupo LEADTECH podem pegar, liberar e resolver solicitações.", size=12, color=C.GREY_600),
                        ], spacing=4),
                        padding=16, border=ft.border.all(1, C.GREY_300), border_radius=12, col={"sm":12,"md":6,"lg":3}
                    ),
                    ft.Container(
    ...
    def _abrir_tab(self) -> ft.Control:
            ...
            form = ft.Card(content=ft.Container(padding=16, content=ft.Column([
                ft.Text("Abrir Solicitação para LEADTECH", size=18, weight=ft.FontWeight.BOLD),
    ...
    def _admin_tab(self) -> ft.Control:
            ...
            def refresh():
                ...
                for u in self.db.list_users():
                    ...
                    chk_leadtech = ft.Checkbox(value=self.db.user_in_group(u.id, "LEADTECH"))
                    ...
                    chk_leadtech.on_change = bind_toggle(u.id, chk_leadtech, "LEADTECH")
                    ...
                    table.rows.append(ft.DataRow(cells=[
                        ...
                        ft.DataCell(ft.Row([ft.Text("SUPORTE"), chk_sup, ft.Text("LEADTECH"), chk_leadtech], spacing=6)),
                        ft.DataCell(ft.Row([
...
def main(page):
    ...
    def after_login(user: User):
        LEADTECHApp(page, db, user, on_logout=show_login)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Solicitações LEADTECH - Desktop/Web")
