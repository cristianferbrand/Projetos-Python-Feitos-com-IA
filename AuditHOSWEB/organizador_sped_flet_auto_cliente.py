...
Como executar:
    python organizador_sped_flet_auto_cliente.py
"""
...
class OrganizadorSPED:
    ...
    async def _wait_file_ready(self, path: Path, timeout: float = 20.0, poll: float = 0.3, stable_reads: int = 2) -> bool:
        """Espera até o arquivo existir, estabilizar o tamanho e permitir leitura (mitiga WinError 32)."""
        import time
    ...
    def _stop_monitor(self):
        ...
        if getattr(self, "monitor_task", None) and not self.monitor_task.done():
            try:
        ...
        if self.monitor_task and not self.monitor_task.done():
            # tarefa será encerrada no próximo ciclo
            pass
        ...
        try:
            while self.monitor_enabled:
                try:
                    ...
                    try:
                        self.log.value = f"Erro no ciclo do monitor: {ex}"
                        self.page.update()
                    except Exception:
                        pass
    ...
    async def _scan_once(self):
        ...
        for p in files:
            ...
            try:
                ...
                continue
            # processa
            ok = await self._process_single_auto(p)
            if ok:
                self.seen.add(sp)

    async def _process_single_auto(self, arquivo: Path) -> bool:
        # Aguarda arquivo liberar (lock/sincronização)
        ready = await self._wait_file_ready(arquivo)
        if not ready:
            # não marca como 'visto'; será tentado no próximo ciclo
            return False
    ...
    def build(self) -> ft.Control:
        ...
        layout = ft.Column([
            ...
            destino_row,
            cfg_row,
            ft.Container(content=ft.Column([ft.Row([self.table], scroll=ft.ScrollMode.AUTO)], scroll=ft.ScrollMode.AUTO, expand=True), height=380, border=ft.border.all(1, op(0.15, C.BLUE)), border_radius=10, padding=10),
            actions,
            ft.Text("Log:"),
            ft.Container(content=ft.Column([self.log], scroll=ft.ScrollMode.AUTO, expand=True), padding=10, border_radius=8, bgcolor=op(0.03, C.BLUE)),
