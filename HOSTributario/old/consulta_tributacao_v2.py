...
# TENTATIVA de importar openpyxl (para Exportar Excel)
try:
...
# (opcional) dar peso para a coluna do botão Excel
frame_param.columnconfigure(6, weight=1)
...
btn_excel.grid(row=0, column=6, rowspan=3, padx=6, pady=10, ipady=10, ipadx=10, sticky='nsew')
...
checkbox_container.grid(row=4, column=0, columnspan=7, sticky='w', padx=10)
...
colunas_todas = [
    "ID", "Código de Barras", "NCM", "CEST", "Tipo", "Lista", "CodANP", "Código Int.", "EX", "CodENQ", "CST IPI Ent", "CST IPI Sai", "Aliq IPI",
...
def exportar_excel():
    if Workbook is None:
        messagebox.showerror(
            "Dependência ausente",
            "A biblioteca 'openpyxl' é necessária para exportar Excel.\n\nInstale com:\n  pip install openpyxl"
        )
        return
    ...
    try:
        ...
        colunas_visiveis = list(tree["columns"])
        ws_res.append(colunas_visiveis)
        for iid in tree.get_children(""):
            ws_res.append([tree.set(iid, c) for c in colunas_visiveis])

        # Aba Colunas
        ws_cols = wb.create_sheet("Colunas")
        ws_cols.append(["Coluna", "Selecionada"])
        for c in colunas_todas:
            selecionada = "Sim" if colunas_visiveis_vars[c].get() else "Não"
            ws_cols.append([c, selecionada])

        wb.save(fpath)
        messagebox.showinfo("OK", f"Arquivo Excel exportado com sucesso:\n{fpath}")
    ...
    def processar():
        try:
            ...
            if response.status_code == 200:
                ...
                # Lista para armazenar códigos sem tributação
                codigos_nao_encontrados = []

                for item in resultado:
                    raw = item.get("tributacao", None)
                    if not raw or str(raw).strip() in ("", "null", "None"):
                        tributacao = {}
                        codigos_nao_encontrados.append(item.get("codigo"))
                    else:
                        try:
                            tributacao = json.loads(raw)
                        except Exception:
                            tributacao = {}
                            codigos_nao_encontrados.append(item.get("codigo"))
                    ...
                    linha = {
                        "ID": item.get("id"), "Código de Barras": item.get("codigo"), "NCM": tributacao.get("ncm"), "CEST": tributacao.get("cest"),
                ...
                if codigos_nao_encontrados:
                    ...
                    try:
                        ...
                        messagebox.showwarning(
                            "Aviso",
                            "Não foi possível salvar o arquivo de códigos não encontrados.\n"
                            f"Erro: {e}\n\nLista:\n" + "\n".join(codigos_nao_encontrados)
                        )
