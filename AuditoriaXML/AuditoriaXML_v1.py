...
def main(page: ft.Page):
    ...
    def set_loading_progress(current: int, total: int):
        try:
            loading_bar.value = max(0.0, min(1.0, (current / total) if total else 0.0))
        except Exception:
            pass
        page.update()
    ...
    def recomputar_gestao():
        ...
        # === NCM (TODOS) – respeita filtros/Tipo ===
        acc_ncm_all = {}
        count_ncm = {}
        for n in filtradas:
            for it in itens_por_chave.get(n["chave"], []):
                key = it.get("NCM") or ""
                acc_ncm_all[key] = acc_ncm_all.get(key, Decimal("0.00")) + it.get("vProd", Decimal("0.00"))
                count_ncm[key] = count_ncm.get(key, 0) + 1
        ncm_rows = []
        for k, v in sorted(acc_ncm_all.items(), key=lambda x: x[1], reverse=True):
            ncm_rows.append(ft.DataRow(cells=[
                ft.DataCell(ft.Text(k)),
                ft.DataCell(ft.Text(brl(v))),
                ft.DataCell(ft.Text(str(count_ncm.get(k,0)))),
            ]))
        ncm_all_table.rows = ncm_rows

        # === CFOP (Entradas) e (Saídas) por (CFOP, ICMS_CST, pICMS) ===
        def agrega_cfop_por_tipo(tipo_alvo: str):
            acc = {}
            cnt = {}
            for n in filtradas:
                ...
                for it in itens_por_chave.get(n["chave"], []):
                    ...
                    key = (cfop, cst, str(aliq))
                    acc[key] = acc.get(key, Decimal("0.00")) + it.get("vProd", Decimal("0.00"))
                    cnt[key] = cnt.get(key, 0) + 1
            return acc, cnt

        acc_e, cnt_e = agrega_cfop_por_tipo("0")
        acc_s, cnt_s = agrega_cfop_por_tipo("1")

        def monta_rows_cfop(acc, cnt):
            rows = []
            for (cfop, cst, aliq), v in sorted(acc.items(), key=lambda x: x[1], reverse=True):
                aliq_txt = br_pct(float(D(aliq)))
                rows.append(ft.DataRow(cells=[
                    ft.DataCell(ft.Text(cfop)),
                    ft.DataCell(ft.Text(cst)),
                    ft.DataCell(ft.Text(aliq_txt)),
                    ft.DataCell(ft.Text(brl(v))),
                    ft.DataCell(ft.Text(str(cnt.get((cfop, cst, aliq), 0)))),
                ]))
            return rows

        cfop_e_table.rows = monta_rows_cfop(acc_e, cnt_e)
        cfop_s_table.rows = monta_rows_cfop(acc_s, cnt_s)

        # === CST PIS/COFINS – agora separados (E/S), apenas vPIS>0 e vCOFINS>0 ===
        def agrega_pc_por_tipo(tipo_alvo: str):
            acc_pc = {}
            cnt_pc = {}
            tot_pis = {}
            tot_cof = {}
            for n in filtradas:
                if n.get("tpNF") != tipo_alvo:
                    continue
                for it in itens_por_chave.get(n["chave"], []):
                    if (it.get("vPIS", Decimal("0.00")) > 0) and (it.get("vCOFINS", Decimal("0.00")) > 0):
                        key = (it.get("PIS_CST") or "", it.get("COFINS_CST") or "")
                        acc_pc[key] = acc_pc.get(key, Decimal("0.00")) + it.get("vProd", Decimal("0.00"))
                        cnt_pc[key] = cnt_pc.get(key, 0) + 1
                        tot_pis[key] = tot_pis.get(key, Decimal("0.00")) + it.get("vPIS", Decimal("0.00"))
                        tot_cof[key] = tot_cof.get(key, Decimal("0.00")) + it.get("vCOFINS", Decimal("0.00"))
            return acc_pc, cnt_pc, tot_pis, tot_cof

        acc_pis_e, cnt_pis_e, tot_pis_e, tot_cof_e = agrega_pc_por_tipo("0")
        acc_pis_s, cnt_pis_s, tot_pis_s, tot_cof_s = agrega_pc_por_tipo("1")

        def monta_rows_pc(acc, cnt, tot_pis, tot_cof):
            rows = []
            for (cst_p, cst_c), vprod in sorted(acc.items(), key=lambda x: x[1], reverse=True):
                rows.append(ft.DataRow(cells=[
        ...
        piscofins_cst_table_e.rows = monta_rows_pc(acc_pis_e, cnt_pis_e, tot_pis_e, tot_cof_e)
        piscofins_cst_table_s.rows = monta_rows_pc(acc_pis_s, cnt_pis_s, tot_pis_s, tot_cof_s)
    ...
    # ======= EXPORTAÇÕES (Gestão Detalhamentos) =======
    def exportar_ncm_todos_csv(_=None):
        filtradas = get_filtered_notas()
        if not filtradas:
            snack("Nada para exportar – verifique filtros.")
            return
        ...
        with open(caminho, "w", newline="", encoding="utf-8") as f:
            w = csv.writer(f, delimiter=';')
            w.writerow(["NCM","vProd_total","itens"])
            for k, v in sorted(acc.items(), key=lambda x: x[1], reverse=True):
                w.writerow([k, br_dec(v), cnt.get(k, 0)])
        snack(f"Exportado: {caminho}")
    ...
    def exportar_cfop_e_csv(_=None):
        ...
        if not filtradas:
            snack("Nada para exportar – verifique filtros.")
            return
        caminho = EXPORT_DIR / "gestao_cfop_entradas.csv"
        acc = {}
        cnt = {}
        for n in filtradas:
            if n.get("tpNF") != "0":
                continue
            for it in itens_por_chave.get(n["chave"], []):
                key = (it.get("CFOP") or "", it.get("ICMS_CST") or "", str(it.get("ICMS_pICMS", Decimal("0.00"))))
                acc[key] = acc.get(key, Decimal("0.00")) + it.get("vProd", Decimal("0.00"))
                cnt[key] = cnt.get(key, 0) + 1
        with open(caminho, "w", newline="", encoding="utf-8") as f:
            w = csv.writer(f, delimiter=';')
            w.writerow(["CFOP","ICMS_CST","pICMS(%)","vProd_total","itens"])
            for (cfop, cst, aliq), v in sorted(acc.items(), key=lambda x: x[1], reverse=True):
                w.writerow([cfop, cst, br_pct(float(D(aliq))), br_dec(v), cnt.get((cfop, cst, aliq), 0)])
        snack(f"Exportado: {caminho}")
    ...
    def exportar_cfop_s_csv(_=None):
        ...
        if not filtradas:
            snack("Nada para exportar – verifique filtros.")
            return
        caminho = EXPORT_DIR / "gestao_cfop_saidas.csv"
        acc = {}
        cnt = {}
        for n in filtradas:
            if n.get("tpNF") != "1":
    ...
    # NOVOS: Exports PIS/COFINS separados
    def exportar_piscofins_cst_csv_e(_=None):
        filtradas = get_filtered_notas()
        if not filtradas:
            snack("Nada para exportar – verifique filtros.")
            return
        caminho = EXPORT_DIR / "gestao_pis_cofins_cst_entradas.csv"
        acc_pc = {}
        cnt_pc = {}
        tot_pis = {}
        tot_cof = {}
        ...
        with open(caminho, "w", newline="", encoding="utf-8") as f:
            w = csv.writer(f, delimiter=';')
            w.writerow(["CST_PIS","CST_COFINS","vPIS_total","vCOFINS_total","vProd_total","itens"])
            for (cst_p, cst_c), vprod in sorted(acc_pc.items(), key=lambda x: x[1], reverse=True):
                w.writerow([
                    cst_p, cst_c,
                    br_dec(tot_pis.get((cst_p, cst_c), Decimal("0.00"))),
                    br_dec(tot_cof.get((cst_p, cst_c), Decimal("0.00"))),
                    br_dec(vprod),
                    cnt_pc.get((cst_p, cst_c), 0),
                ])
        snack(f"Exportado: {caminho}")
    ...
    def exportar_piscofins_cst_csv_s(_=None):
        ...
        if not filtradas:
            snack("Nada para exportar – verifique filtros.")
            return
        caminho = EXPORT_DIR / "gestao_pis_cofins_cst_saidas.csv"
        acc_pc = {}
        cnt_pc = {}
        tot_pis = {}
        tot_cof = {}
    ...
    kpi_cards_geral = ft.Row(
        controls=[
            ...
            _kpi_card("% Notas com Divergência", kpi_diverg_pct, C.AMBER_200, C.AMBER_200),
        ],
    ...
    home_view = ft.View(
        ...
        controls=[
            ...
            ft.Divider(height=12, color=op(0.2, C.GREY)),
            botoes,
            ft.Container(content=ft.Column([tabela_dados], expand=True, scroll=ft.ScrollMode.AUTO), expand=True),
            ft.Divider(height=12, color=op(0.2, C.GREY)),
    ...
    itens_view = ft.View(
        ...
        controls=[
            ...
            ft.Divider(height=8, color=op(0.2, C.GREY)),
            ft.Container(content=ft.Column([itens_table], expand=True, scroll=ft.ScrollMode.AUTO), expand=True),
        ],
        padding=8,
    )
