...
def main(page: ft.Page):
        ...
        view = ft.View(
            ...
            controls=[
                ...
                ft.Container(
                    ...
                    ], tight=False, spacing=10, scroll=ft.ScrollMode.AUTO, expand=True),
                    padding=16,
                    expand=True,
                ),
            ],
    ...
    def route_change(e: ft.RouteChangeEvent):
        if e.route.startswith("/simular/"):
            # Não destrói o stack quando estiver na rota de simulação
            page.update()
            return

        # Reconstrói a view raiz para outras rotas ("/")
        page.views.clear()
        main_view = ft.View(
            route="/",
            controls=[
    ...
    if os.path.isfile(settings.get("class_json_path", "")):
        ...
        if os.path.isfile(DEFAULT_CLASS_JSON):
            ...
            local_json = os.path.join(APP_DIR, "classificacao_tributaria.json")
            if os.path.isfile(local_json):
                log(f"[INFO] Usando JSON local: {local_json}")
                load_classes_from(local_json)
            else:
