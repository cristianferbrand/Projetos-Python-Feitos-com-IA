...
def open_adjust_cnpj():
    ...
    tk.Button(adjust_window, text="Iniciar Ajuste", command=lambda: start_processing(entry_adjust_input)).pack(pady=20)
...
def open_verificar_ajustar_c170():
    """ Abre a janela para Verificação e Ajuste do Registro C170 """
    adjust_c170_window = tk.Toplevel(root)
    adjust_c170_window.title("Verificação e Ajuste de C170")
    adjust_c170_window.geometry("800x600")
    ...
    tk.Button(frame_botoes, text="Processar",
              command=lambda: processar_arquivos_verificar_ajustar(adjust_c170_window, entrada_text),
              width=20, bg="#4CAF50", fg="white", font=("Arial", 12, "bold")).pack(pady=10)

def processar_arquivos_verificar_ajustar(window, entrada_text):
    """ Processa os arquivos selecionados para verificar e ajustar os registros C170 """
    try:
        for arquivo_entrada in window.lista_arquivos:
            verificar_e_ajustar_c170(arquivo_entrada)

        # Limpar a lista dentro da janela e atualizar a interface
        window.lista_arquivos.clear()
        atualizar_texto_entrada(window.lista_arquivos, entrada_text)

        messagebox.showinfo("Concluído", "Todos os arquivos foram verificados e ajustados com sucesso.")
    except Exception as e:
        messagebox.showerror("Erro", f"Ocorreu um erro: {e}")

def verificar_e_ajustar_c170(arquivo_entrada):
    """ Verifica e ajusta os registros C170 conforme regras do CST PIS/COFINS """
    registros_c170 = []
    registros_ajustados = []
    ncm_dict = {}
                    ...
                    if len(campos) > 31:
                        ...
                        if cst_pis == '50' and cst_cofins == '50':
                            ...
                            if any(ncm.startswith(prefix) for prefix in ncm_farmaceuticos_prefixos) and ncm not in ncm_excecoes_farmaceuticos:
                                registros_c170.append(f"COD_ITEM: {cod_item}, NCM: {ncm} (Farmacêutico)")
                            elif any(ncm.startswith(prefix) for prefix in ncm_perfumaria_prefixos):
                                registros_c170.append(f"COD_ITEM: {cod_item}, NCM: {ncm} (Perfumaria)")
                            else:
                                registros_c170.append(f"COD_ITEM: {cod_item}, NCM: {ncm} (Outros)")

                            # Ajuste automático dos registros C170
...
def selecionar_arquivos_entrada(window, entrada_text):
    """ Seleciona arquivos e atualiza a lista da janela """
    caminhos = filedialog.askopenfilenames(title="Selecione os arquivos para Verificação e Ajuste de C170", filetypes=[("Arquivos TXT", "*.txt")])
    if caminhos:
        window.lista_arquivos.extend(caminhos)  # Atualiza a lista da janela
        atualizar_texto_entrada(window.lista_arquivos, entrada_text)

def atualizar_texto_entrada(lista_arquivos, entrada_text):
    """ Atualiza o widget de texto para exibir a lista de arquivos selecionados """
    entrada_text.delete("1.0", tk.END)  # Limpa o conteúdo anterior
    entrada_text.insert(tk.END, "\n".join(lista_arquivos))  # Insere os arquivos na área de texto
...
def open_verificar_numeros_faltantes():
    ...
    verificar_window.geometry("450x350")
    
    tk.Label(verificar_window, text="Período Início (YYYY-MM-DD):").pack()
    entry_inicio = tk.Entry(verificar_window)
    entry_inicio.pack()
    
    tk.Label(verificar_window, text="Período Fim (YYYY-MM-DD):").pack()
    entry_fim = tk.Entry(verificar_window)
    entry_fim.pack()
    
    tk.Label(verificar_window, text="Série:").pack()
    entry_serie = tk.Entry(verificar_window)
    entry_serie.pack()
    
    tk.Label(verificar_window, text="Empresa:").pack()
    entry_empresa = tk.Entry(verificar_window)
    entry_empresa.pack()
    
    tk.Label(verificar_window, text="Banco de Dados Firebird:").pack()
    entry_banco = tk.Entry(verificar_window)
    entry_banco.insert(0, "C:/Mercfarma/CADASTRO.FDB")
    entry_banco.pack()
    
    tk.Button(verificar_window, text="Selecionar Banco", command=lambda: selecionar_banco(entry_banco)).pack()
    
    progress_bar = ttk.Progressbar(verificar_window, orient=tk.HORIZONTAL, length=300, mode='determinate')
    progress_bar.pack(pady=10)
    
    tk.Button(verificar_window, text="Verificar", command=lambda: verificar_numeros_faltantes(entry_inicio, entry_fim, entry_serie, entry_empresa, entry_banco, progress_bar)).pack()
...
def verificar_numeros_faltantes(entry_inicio, entry_fim, entry_serie, entry_empresa, entry_banco, progress_bar):
    def processar():
        try:
...
if sistema_pode_abrir:
    ...
    tk.Label(root, text="Selecione uma funcionalidade:", font=("Arial", 14, "bold")).pack(pady=20)

    tk.Button(root, text="Organizador de Arquivos SPED'S", command=open_organizador_sped, width=60, bg="#1976d2", fg="white", font=("Arial", 12, "bold")).pack(pady=10)
    tk.Button(root, text="Ajustar Unidade Registro C170", command=open_adjust_c170, width=60, bg="#1976d2", fg="white", font=("Arial", 12, "bold")).pack(pady=10)
    tk.Button(root, text="Verificação e Ajuste CST Pis Cofins Registro C170", command=open_verificar_ajustar_c170, width=60, bg="#2e7d32", fg="white", font=("Arial", 12, "bold")).pack(pady=10)
    tk.Button(root, text="Ajuste Rigistro C170 COD_CTA", command=open_cod_cta_adjuster, width=60, bg="#2e7d32", fg="white", font=("Arial", 12, "bold")).pack(pady=10)
    tk.Button(root, text="Totalizador XML por CFOP e Alíquota", command=open_totalizador_xml, width=60, bg="#1976d2", fg="white", font=("Arial", 12, "bold")).pack(pady=10)
    tk.Button(root, text="Extrair Registros C100", command=open_extrator_sped, width=60, bg="#1976d2", fg="white", font=("Arial", 12, "bold")).pack(pady=10)
    tk.Button(root, text="Comparador de Arquivos CSV", command=open_comparador_csv, width=60, bg="#1976d2", fg="white", font=("Arial", 12, "bold")).pack(pady=10)
    tk.Button(root, text="Gerador Registro A100", command=open_sped_a100, width=60, bg="#2e7d32", fg="white", font=("Arial", 12, "bold")).pack(pady=10)
    tk.Button(root, text="Ajuste CNPJ do PIX - SPED - (Em Desenvolvimento)", command=open_adjust_cnpj, width=60, bg="#1976d2", fg="white", font=("Arial", 12, "bold")).pack(pady=10)
    tk.Button(root, text="Verificar Números Faltantes Firebird", command=open_verificar_numeros_faltantes, width=60, bg="#1976d2", fg="white", font=("Arial", 12, "bold")).pack(pady=10)
