...
def analisar_arquivo():
    ...
    if not arquivo_selecionado:
        ...
        return
    farmaceuticos, perfumaria, excecoes, insumos_aliqzero = verificar_e_ajustar_c170(arquivo_selecionado, ajustar=False)
    atualizar_resultado(farmaceuticos, perfumaria, excecoes, insumos_aliqzero)
...
def processar_arquivo_c170_piscofins():
    ...
    if not arquivo_selecionado:
        ...
        return
    farmaceuticos, perfumaria, excecoes, insumos_aliqzero = verificar_e_ajustar_c170(arquivo_selecionado, ajustar=True)
    atualizar_resultado(farmaceuticos, perfumaria, excecoes, insumos_aliqzero)
...
def verificar_e_ajustar_c170(arquivo_entrada, ajustar=False):
    registros_ajustados = []
    ncm_dict = {}
    farmaceuticos, perfumaria, excecoes, insumos_aliqzero = [], [], [], []
            ...
            if len(campos) > 1:
                if campos[1] == '0200':
                    ...
                    if len(campos) > 31:
                        ...
                        # Se CFOP for um dos da lista e CST_PIS e CST_COFINS forem 50
                        if cfop in cfops_ajuste_99 and cst_pis == '50' and cst_cofins == '50':
                            if ajustar:
                                campos[25] = '99'
                                campos[31] = '99'

                        # Verificação por NCM e regras já existentes
    ...
    return farmaceuticos, perfumaria, excecoes, insumos_aliqzero

def atualizar_resultado(farmaceuticos, perfumaria, excecoes, insumos_aliqzero):
    text_area.config(state=tk.NORMAL)
    text_area.delete('1.0', tk.END)
    resultado = "Produtos Farmacêuticos Sujeitos a Incidência Monofasica:\n\n" + "\n".join(farmaceuticos) + "\n\n" if farmaceuticos else "Nenhum item farmacêutico encontrado.\n\n"
    resultado += "Produtos de Perfumaria Sujeitos a Incidência Monofasica:\n\n" + "\n".join(perfumaria) + "\n\n" if perfumaria else "Nenhum item de perfumaria encontrado.\n\n"
    resultado += "Produtos Sujeitos à Alíquota Zero:\n\n" + "\n".join(insumos_aliqzero) + "\n\n" if insumos_aliqzero else "Nenhum item de aliquota zero encontrado.\n\n"
