...
def open_totalizador_xml():
    ...
    totalizador_window.geometry("1200x700")

    namespaces = {'nfe': 'http://www.portalfiscal.inf.br/nfe'}

    # Criar Treeviews
    def create_treeview(parent, title, columns):
    ...
    total_modelo_tree = create_treeview(totalizador_window, "Totais por Modelo", 
                                        ("Modelo", "Total vProd", "Total vBC", "Total vICMS"))
    
    totals_tree = create_treeview(totalizador_window, "Totais por CFOP", 
                                  ("CFOP", "Total vProd", "Total vBC", "Total vICMS", "Aliquota ICMS(%)"))

    pis_cofins_tree = create_treeview(totalizador_window, "Totais por CFOP e CST - PIS/COFINS",
                                      ("CFOP", "CST", "Aliquota ICMS(%)", "Alíquota PIS (%)", "Alíquota COFINS (%)", 
                                       "Total vProd", "vBC do PIS", "vBC do COFINS"))

    # Criar dicionários para totalização
    modelo_totals = defaultdict(lambda: {'vBC': 0.0, 'vICMS': 0.0, 'vProd': 0.0, 'pICMS': 0.0})
    cfop_totals = defaultdict(lambda: {'vBC': 0.0, 'vICMS': 0.0, 'vProd': 0.0, 'pICMS': 0.0})
    pis_cofins_totals = defaultdict(lambda: {'vBC_PIS': 0.0, 'vBC_COFINS': 0.0, 'pPIS': 0.0, 'pCOFINS': 0.0, 'vProd': 0.0})
    ...
    for file_name in xml_files:
        ...
        try:
            ...
            print(f"Erro ao processar XML: {file_path}")
            continue

        modelo_nota = root_xml.find('.//nfe:NFe/nfe:infNFe/nfe:ide/nfe:mod', namespaces)
        modelo_nota = modelo_nota.text if modelo_nota is not None else "Desconhecido"

        for det in root_xml.findall('.//nfe:NFe/nfe:infNFe/nfe:det', namespaces):
            cfop = det.find('.//nfe:prod/nfe:CFOP', namespaces)
            cfop = cfop.text if cfop is not None else "0000"

            vProd = det.find('.//nfe:prod/nfe:vProd', namespaces)
            vProd = float(vProd.text) if vProd is not None else 0.0

            icms_element = det.find('.//nfe:imposto/nfe:ICMS', namespaces)
            vBC = sum(float(el.text) if el is not None else 0.0 for el in icms_element.findall('.//nfe:vBC', namespaces)) if icms_element is not None else 0.0
            vICMS = sum(float(el.text) if el is not None else 0.0 for el in icms_element.findall('.//nfe:vICMS', namespaces)) if icms_element is not None else 0.0
            # Adicionar a alíquota do ICMS
            pICMS = sum(float(el.text) if el is not None else 0.0 for el in icms_element.findall('.//nfe:pICMS', namespaces)) if icms_element is not None else 0.0

            modelo_totals[modelo_nota]['vBC'] += vBC
            modelo_totals[modelo_nota]['vICMS'] += vICMS
            modelo_totals[modelo_nota]['vProd'] += vProd
            modelo_totals[modelo_nota]['pICMS'] = pICMS  # Atualizar alíquota média

            cfop_totals[cfop]['vBC'] += vBC
            cfop_totals[cfop]['vICMS'] += vICMS
            cfop_totals[cfop]['vProd'] += vProd
            cfop_totals[cfop]['pICMS'] = pICMS  # Atualizar alíquota média

            pis_element = det.find('.//nfe:imposto/nfe:PIS/nfe:PISAliq', namespaces)
            cofins_element = det.find('.//nfe:imposto/nfe:COFINS/nfe:COFINSAliq', namespaces)

            if pis_element is not None and cofins_element is not None:
                cst = pis_element.find('.//nfe:CST', namespaces)
                cst = cst.text if cst is not None else "N/A"

                vBC_PIS = float(pis_element.find('.//nfe:vBC', namespaces).text or 0.0)
                pPIS = float(pis_element.find('.//nfe:pPIS', namespaces).text or 0.0)
                vBC_COFINS = float(cofins_element.find('.//nfe:vBC', namespaces).text or 0.0)
                pCOFINS = float(cofins_element.find('.//nfe:pCOFINS', namespaces).text or 0.0)

                pis_cofins_totals[(cfop, cst)]['vBC_PIS'] += vBC_PIS
                pis_cofins_totals[(cfop, cst)]['vBC_COFINS'] += vBC_COFINS
                pis_cofins_totals[(cfop, cst)]['pPIS'] = pPIS
                pis_cofins_totals[(cfop, cst)]['pCOFINS'] = pCOFINS
                pis_cofins_totals[(cfop, cst)]['vProd'] += vProd
                pis_cofins_totals[(cfop, cst)]['pICMS'] = pICMS  # Adicionando o pICMS ao totalizador de PIS/COFINS

    # Preencher Treeviews com os dados
    # Atualizar os Treeviews para incluir a alíquota do ICMS
    for modelo, totals in modelo_totals.items():
        total_modelo_tree.insert('', 'end', values=(modelo, f"{totals['vProd']:.2f}", f"{totals['vBC']:.2f}", f"{totals['vICMS']:.2f}", f"{totals['pICMS']:.2f}"))

    for cfop, totals in cfop_totals.items():
        totals_tree.insert('', 'end', values=(cfop, f"{totals['vProd']:.2f}", f"{totals['vBC']:.2f}", f"{totals['vICMS']:.2f}", f"{totals['pICMS']:.2f}"))

    for (cfop, cst), totals in pis_cofins_totals.items():
        pis_cofins_tree.insert('', 'end', values=(cfop, cst, f"{totals['pICMS']:.2f}", f"{totals['pPIS']:.2f}", f"{totals['pCOFINS']:.2f}", f"{totals['vProd']:.2f}", f"{totals['vBC_PIS']:.2f}", f"{totals['vBC_COFINS']:.2f}"))
...
def open_sped_a100():
    ...
    tk.Button(sped_a100_window, text="Selecionar Diretório de XML e Processar", 
              command=lambda: select_directory_sped(entry_aliquota_pis.get(), entry_aliquota_cofins.get(), output_sped)).pack(pady=10)
...
def process_directory_gerador_sped(directory_path, aliquota_pis, aliquota_cofins, output_widget):
...
def select_directory_sped(aliquota_pis, aliquota_cofins, output_widget):
    directory_path = filedialog.askdirectory()
    if directory_path:
        try:
            aliquota_pis = float(aliquota_pis.replace(",", "."))
            aliquota_cofins = float(aliquota_cofins.replace(",", "."))
            process_directory_gerador_sped(directory_path, aliquota_pis, aliquota_cofins, output_widget)
        except ValueError:
    ...
    for txt_file in txt_files:
        ...
        if match:
            ...
            if not linha_cliente.empty:
                crm = linha_cliente.iloc[0]['CRM']
                fantasia = linha_cliente.iloc[0]['FANTASIA']
                nome_pasta = f"{crm} - {fantasia}".strip()
...
def analisar_arquivo():
    ...
    if not arquivo_selecionado:
        ...
        return
    itens_cfops_ajust99, farmaceuticos, perfumaria, excecoes, insumos_aliqzero = verificar_e_ajustar_c170(arquivo_selecionado, ajustar=False)
    atualizar_resultado(itens_cfops_ajust99, farmaceuticos, perfumaria, excecoes, insumos_aliqzero)

def processar_arquivo_c170_piscofins():
    global arquivo_selecionado
    if not arquivo_selecionado:
        messagebox.showwarning("Aviso", "Nenhum arquivo foi selecionado para processamento!")
        return
    itens_cfops_ajust99, farmaceuticos, perfumaria, excecoes, insumos_aliqzero = verificar_e_ajustar_c170(arquivo_selecionado, ajustar=True)
    atualizar_resultado(itens_cfops_ajust99, farmaceuticos, perfumaria, excecoes, insumos_aliqzero)
...
def verificar_e_ajustar_c170(arquivo_entrada, ajustar=False):
    registros_ajustados = []
    ncm_dict = {}
    itens_cfops_ajust99, farmaceuticos, perfumaria, excecoes, insumos_aliqzero = [], [], [], [], []
    
    # Definições de NCMs para cada categoria
    ...
    ncm_insumos_aliqzero = {"3306", "34011190", "48181000", "22011000"}
    ncm_perfumaria_prefixos = {"3303", "3304", "3305", "3306", "3307", "34011190", "34012010", "96032100"}

    # Lista de CFOPs que precisam do ajuste CST para 99
    cfops_ajuste_99 = {'1114', '1910', '2910', '2911'}
    ...
    try:
        ...
        for linha in linhas:
            ...
            if len(campos) > 1:
                if campos[1] == '0200':
                ...
                elif campos[1] == 'C170' and len(campos) > 31:
                    ...
                    # **Verificação por CFOP**
                    if cfop in cfops_ajuste_99 and cst_pis == '50' and cst_cofins == '50':
                        itens_cfops_ajust99.append(f"COD_ITEM: {cod_item}, CFOP: {cfop}, CST_PIS: {cst_pis}, CST_COFINS: {cst_cofins}")
                        if ajustar:
                            campos[25] = '99'
                            campos[31] = '99'
                    ...
                    if cst_pis == '50' and cst_cofins == '50':
                        ...
                        elif any(ncm.startswith(prefix) for prefix in ncm_insumos_aliqzero):
                            insumos_aliqzero.append(f"COD_ITEM: {cod_item}, NCM: {ncm}")
                            if ajustar:
                                campos[25] = '70'
                                campos[31] = '70'
                                campos[26] = '0'
    ...
    return itens_cfops_ajust99, farmaceuticos, perfumaria, excecoes, insumos_aliqzero

def atualizar_resultado(itens_cfops_ajust99, farmaceuticos, perfumaria, excecoes, insumos_aliqzero):
    text_area.config(state=tk.NORMAL)
    text_area.delete('1.0', tk.END)

    resultado = ""

    if itens_cfops_ajust99:
        resultado += "Produtos com CST Pis/Cofins Incompatível com CFOP (Operações Geradoras de Credito):\n\n" + "\n".join(itens_cfops_ajust99) + "\n\n"
    else:
        resultado += "Nenhum item com CFOP incompatível encontrado.\n\n"

    if farmaceuticos:
        resultado += "Produtos Farmacêuticos Sujeitos a Incidência Monofásica:\n\n" + "\n".join(farmaceuticos) + "\n\n"
    else:
        resultado += "Nenhum item farmacêutico encontrado.\n\n"

    if perfumaria:
        resultado += "Produtos de Perfumaria Sujeitos a Incidência Monofásica:\n\n" + "\n".join(perfumaria) + "\n\n"
    else:
        resultado += "Nenhum item de perfumaria encontrado.\n\n"

    if insumos_aliqzero:
        resultado += "Produtos Sujeitos à Alíquota Zero:\n\n" + "\n".join(insumos_aliqzero) + "\n\n"
    else:
        resultado += "Nenhum item de alíquota zero encontrado.\n\n"

    if excecoes:
        resultado += "Itens para analisar:\n\n" + "\n".join(excecoes) + "\n\n"
    else:
        resultado += "Nenhum item encontrado para analisar.\n\n"
...
def open_verificar_ncm():
    ...
    tk.Button(frame, text="Processar", command=processar_arquivo_c170_piscofins, bg="#4CAF50", fg="white", font=("Arial", 12, "bold"), width=25).pack(pady=5)
...
def gerar_senha():
    data_atual = datetime.now().strftime("%Y-%m-%d")  # Obtém a data no formato YYYY-MM-DD
...
if sistema_pode_abrir:
    ...
    tk.Button(frame_ferramentas, text="Totalizador XML por CFOP e Alíquota", command=open_totalizador_xml, width=60, bg="#eceff1", fg="black", font=("Arial", 12, "bold")).pack(pady=5)
