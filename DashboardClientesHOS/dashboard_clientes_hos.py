...
python "dashboard_clientes_hos_v2.py" --web --host 127.0.0.1 --port 8558 --view BROWSER
"""
...
class Breakpoints:
    XS = 480      # phones compactos
    SM = 768      # phones grandes / tablets em pé
    MD = 1024     # tablets paisagem / notebooks 13"
    LG = 1440     # desktops comuns
    XL = 1920     # ultrawide/monitores grandes

def vp_size(page):
    try:
        return int(page.width or 0), int(page.height or 0)
    except Exception:
        return 1200, 800

def vp_is_xs(w: int) -> bool: return w < Breakpoints.XS
def vp_is_sm(w: int) -> bool: return Breakpoints.XS <= w < Breakpoints.SM
def vp_is_md(w: int) -> bool: return Breakpoints.SM <= w < Breakpoints.MD
def vp_is_lg(w: int) -> bool: return Breakpoints.MD <= w < Breakpoints.LG
def vp_is_xl(w: int) -> bool: return w >= Breakpoints.LG

def ui_density_for_width(w: int) -> dict:
    if vp_is_xs(w):
        return {"title": 16, "text": 12, "legend_cols": 1, "pad": 8}
    if vp_is_sm(w):
        return {"title": 18, "text": 13, "legend_cols": 2, "pad": 10}
    if vp_is_md(w):
        return {"title": 18, "text": 14, "legend_cols": 3, "pad": 12}
    return {"title": 20, "text": 14, "legend_cols": 4, "pad": 14}

def map_height_for_viewport(w: int, h: int) -> int:
    if vp_is_xs(w):
        return max(320, int(h * 0.45))
    if vp_is_sm(w):
        return max(360, int(h * 0.50))
    if vp_is_md(w):
        return max(420, int(h * 0.55))
    return max(480, int(h * 0.60))

def table_height_for_viewport(w: int, h: int, factor: float = 0.28, min_h: int = 180, max_h: int = 360) -> int:
    base = int(h * factor)
    if vp_is_xs(w): base = int(h * 0.30)
    if vp_is_sm(w): base = int(h * 0.28)
    return max(min_h, min(max_h, base))
# =============================
...
# Host padrão para construir URLs locais do servidor de mapas (pode ser sobrescrito via --host)
SERVER_HOST = "127.0.0.1"
MAP_HTTP_HOST = "127.0.0.1"  # host de bind do servidor de mapas
...
def ensure_map_http_server():
    ...
    with _MAP_HTTPD_LOCK:
        ...
        for port_try in (8765, 8766, 0):
            try:
                httpd = _SilentTCPServer((MAP_HTTP_HOST, port_try), handler)
                MAP_HTTPD = httpd
...
def embed_folium_map(page, html_str: str, *, altura: int | None = None, height: int | None = None):
    ...
    http_url = f"http://{SERVER_HOST}:{port}/mapa_folium_tmp.html" if port else None
...
# Tema inicial (Material 3 + Dark/Light)
    ...
    def update_uf_legend():
        ...
        try:
            ...
            w, h = vp_size(page)
            dens = ui_density_for_width(w)
            col_count = max(1, min(4, int(dens["legend_cols"])))
    ...
    # Ajuste dinâmico de alturas dos cards conforme viewport
    _w, _h = vp_size(page)
    try: rep_card.content.controls[1].height     = table_height_for_viewport(_w, _h, factor=0.32, min_h=180, max_h=380)
    except Exception: pass
    try: sem_hos_card.content.controls[1].height = table_height_for_viewport(_w, _h, factor=0.22, min_h=140, max_h=260)
    except Exception: pass
    try: mod_card.content.controls[1].height     = table_height_for_viewport(_w, _h, factor=0.28, min_h=160, max_h=320)
    except Exception: pass
    try: cidades_card.content.controls[1].height = table_height_for_viewport(_w, _h, factor=0.30, min_h=160, max_h=360)
    except Exception: pass
    ...
    def recalc_layout():
        try:
            w, h = vp_size(page)
            dens = ui_density_for_width(w)
            page.padding = dens.get("pad", 12)
            page.scroll = ft.ScrollMode.ALWAYS

            # Altura do mapa
            try:
                new_map_h = map_height_for_viewport(w, h)
                mapa_container.height = new_map_h
            except Exception:
                pass

            # Ajustar heights dos cards
            try:
                rep_card.content.controls[1].height     = table_height_for_viewport(w, h, factor=0.32, min_h=180, max_h=380)
            except Exception: pass
            try:
                sem_hos_card.content.controls[1].height = table_height_for_viewport(w, h, factor=0.22, min_h=140, max_h=260)
            except Exception: pass
            try:
                mod_card.content.controls[1].height     = table_height_for_viewport(w, h, factor=0.28, min_h=160, max_h=320)
            except Exception: pass
            try:
                cidades_card.content.controls[1].height = table_height_for_viewport(w, h, factor=0.30, min_h=160, max_h=360)
            except Exception: pass

            # Recalcular legenda (colunas dinâmicas)
            try:
                update_uf_legend()
            except Exception:
                pass
    ...
    ], expand=True, spacing=12, scroll=ft.ScrollMode.ALWAYS))

    def _on_resize_handler(e=None):
        if state.get('settings_open', False):
            return
        # Recalcula layout e depois atualiza o mapa (debounced)
        recalc_layout()
        resize_deb.call(lambda: safe_call_ui(page, refresh_map_only))

    page.on_resize = _on_resize_handler
    # Primeira render
...
if __name__ == "__main__":
    ...
    # Host de bind do servidor de mapas: para acesso externo, use o mesmo host passado por --host;
    # se for '0.0.0.0' ou '::', aceita conexões em todas interfaces.
    if args.host in ("0.0.0.0", "::"):
        MAP_HTTP_HOST = args.host
    elif args.host:
        MAP_HTTP_HOST = args.host
    else:
        MAP_HTTP_HOST = "127.0.0.1"
    # Define o host usado para compor URLs locais; se estiver '0.0.0.0' ou '::', usa 127.0.0.1 para o cliente local
    SERVER_HOST = (args.host if args.host not in ("0.0.0.0", "::", None, "") else "127.0.0.1")
    # Define o host usado para compor URLs locais; se estiver '0.0.0.0' ou '::', usa 127.0.0.1 para o cliente local
    if args.web:
